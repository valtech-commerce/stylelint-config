//--------------------------------------------------------
//-- stylelint config - Standard tests
//--------------------------------------------------------
...
Object.values(tester.subpackages).forEach((root) => {
	...
	describe(`Validate ${tester.getReadablePath(configPath)}`, () => {
		...
		test(`Ensure name is valid`, () => {
			expect(config.name).toMatch(/^@absolunet\/stylelint-config-(?<kebab1>[a-z][a-z0-9]*)(?<kebab2>-[a-z0-9]+)*$/u);
		});

		test(`Ensure keywords are valid`, () => {
			expect(config.keywords).toIncludeAllMembers(['stylelint', 'stylelintconfig', 'stylelint-config']);
		});
	});
});
